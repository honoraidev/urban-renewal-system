...
/* ================= 總覽式審核介面(取代舊的「一次一筆地號、分3步驟」精靈流程) =================
   進來的是同一份 titleDeedWizard.data,只是改成一頁全部列出來審核、最後一次送出建立,
   不用逐筆點「下一步」。左邊是原圖預覽(縮圖列+主圖,用 pdf.js 畫出真正的頁面縮圖),
   右邊分「辨識結果/未辨識項目/匯入設定」三個分頁。

   下面這個區塊完全取代 renderWizardStepParcelEditor/renderWizardStepBuildingEditor 那一整
   套逐筆子步驟(renderParcel*SubStep / renderBuilding*SubStep / advanceFromParcel /
   wizardSplitBodyHtml / renderWizardSplitStep / wizardViewerPaneHtml 等)- 那些函式先保留
   沒刪,做安全網,確認這個新介面沒問題之後可以整批清掉。 */
...
// file_page_counts 缺失時(理論上不會,但防呆)退回「就是第1份檔案第N頁」,不要整個崩掉。
...
async function wizardRenderCurrentPage() {
  const canvas = document.getElementById("wizard-ov-canvas");
  if (!canvas) return;
...
async function wizardRenderThumbnails() {
  const wrap = document.getElementById("wizard-ov-thumbs");
  if (!wrap) return;
...
// 「辨識結果/未辨識項目」的統計數字:對每筆地號/建號的核心欄位跟每位所有權人的姓名/
// 戶籍地址做「有沒有填」的檢查,回傳每一項的通過/未通過狀態,供分頁計數跟未辨識清單用。
// 統一編號故意不檢查空白與否 - 二類謄本本來就把它遮罩成「A123*****6」,不是缺資料。
function wizardComputeFieldChecks() {
  const groups = wizardGroupList();
  const items = [];
...
function wizardOverviewGroupCardHtml(group) {
  if (group.type === "parcel") {
    const p = group.record;
    const gi = group.index;
    return `
      <div class="wizard-ov-card" data-group-type="parcel" data-group-index="${gi}">
        <div class="wizard-ov-card-header">
          <h4>📄 地號 ${escapeHtml(p.parcel_number) || `(第${gi + 1}筆)`}</h4>
          <button type="button" class="btn-danger btn-sm wizard-ov-delete-group" data-group-type="parcel" data-group-index="${gi}">刪除此筆</button>
        </div>
        <div class="field-row">
          <div class="field"><label>鄉鎮市區</label><input data-g-field="township" value="${escapeHtml(p.township)}" autocomplete="off"></div>
          <div class="field"><label>地段</label><input data-g-field="section" value="${escapeHtml(p.section)}" autocomplete="off"></div>
          <div class="field"><label>小段</label><input data-g-field="subsection" value="${escapeHtml(p.subsection)}" autocomplete="off"></div>
          <div class="field"><label>地號</label><input data-g-field="parcel_number" value="${escapeHtml(p.parcel_number)}" autocomplete="off"></div>
          <div class="field" style="flex:0 0 130px"><label>土地面積(㎡)</label><input data-g-field="area_sqm" data-g-numeric type="number" step="0.01" value="${escapeHtml(p.area_sqm)}" autocomplete="off"></div>
        </div>
        <div class="wizard-ov-subhead">所有權人</div>
        <div id="wizard-ov-owners-parcel-${gi}"></div>
        <div class="wizard-ov-subhead">土地他項權利部</div>
        <div id="wizard-ov-enc-parcel-${gi}"></div>
      </div>`;
  }
  const b = group.record;
  const gi = group.index;
  const accArea = (b.accessories || []).reduce((s, a) => s + (Number(a.area_sqm) || 0), 0);
  const totalArea = (Number(b.total_area_sqm) || 0) + accArea || Number(b.floor_area_sqm) || 0;
  return `
    <div class="wizard-ov-card" data-group-type="building" data-group-index="${gi}">
      <div class="wizard-ov-card-header">
        <h4>🏠 建號 ${escapeHtml(b.building_number) || `(第${gi + 1}筆)`}</h4>
        <button type="button" class="btn-danger btn-sm wizard-ov-delete-group" data-group-type="building" data-group-index="${gi}">刪除此筆</button>
      </div>
      <div class="field-row">
        <div class="field"><label>地號</label><input data-g-field="parcel_number" value="${escapeHtml(b.parcel_number)}" autocomplete="off"></div>
        <div class="field"><label>建號</label><input data-g-field="building_number" value="${escapeHtml(b.building_number)}" autocomplete="off"></div>
        <div class="field"><label>建物門牌</label><input data-g-field="building_address" value="${escapeHtml(b.building_address)}" autocomplete="off"></div>
        <div class="field" style="flex:0 0 100px"><label>層數</label><input data-g-field="total_floors" value="${escapeHtml(b.total_floors)}" autocomplete="off"></div>
        <div class="field" style="flex:0 0 130px"><label>建物總面積(㎡)</label><div style="${READONLY_BOX_STYLE}">${totalArea.toFixed(2)}</div></div>
      </div>
      <div class="helper-text" style="margin:-4px 0 8px">樓層/附屬建物明細請用下方「重新辨識」重新上傳這份謄本核對,或到案件「整合清冊」頁面編輯。</div>
      <div class="wizard-ov-subhead">所有權人</div>
      <div id="wizard-ov-owners-building-${gi}"></div>
      <div class="wizard-ov-subhead">建物他項權利部</div>
      <div id="wizard-ov-enc-building-${gi}"></div>
    </div>`;
}

function wizardOverviewResultsHtml() {
  const groups = wizardGroupList();
  if (!groups.length) return `<div class="empty-state">沒有辨識到任何地號或建號資料</div>`;
  return groups.map(wizardOverviewGroupCardHtml).join("");
}

function wizardOverviewIssuesHtml() {
...
function wizardOverviewSettingsHtml() {
  const d = titleDeedWizard.data || {};
  return `
...
function wizardOverviewTabContentHtml() {
  const tab = titleDeedWizard.overviewTab || "results";
  if (tab === "issues") return wizardOverviewIssuesHtml();
  if (tab === "settings") return wizardOverviewSettingsHtml();
  return wizardOverviewResultsHtml();
}

function wizardOverviewBodyHtml() {
  const d = titleDeedWizard.data || {};
  ...
  const tab = titleDeedWizard.overviewTab || "results";
  return `
    <div class="wizard-ov-layout">
      <div class="wizard-ov-viewer">
        <div class="wizard-ov-viewer-toolbar">
          <button type="button" class="btn-sm btn-secondary" id="wizard-ov-zoom-out" title="縮小">－</button>
          <span class="wizard-ov-page-input-wrap">
            <input type="number" id="wizard-ov-page-input" value="${titleDeedWizard.viewerGlobalPage || 1}" min="1" max="${wizardTotalPages()}">
            / ${wizardTotalPages()} 頁
          </span>
          <button type="button" class="btn-sm btn-secondary" id="wizard-ov-zoom-in" title="放大">＋</button>
          <button type="button" class="btn-sm btn-secondary" id="wizard-ov-rotate" title="旋轉">⟳</button>
          <button type="button" class="btn-sm btn-secondary" id="wizard-ov-download" title="下載目前這份檔案" style="margin-left:auto">⬇ 下載</button>
        </div>
        <div class="wizard-ov-viewer-canvas-wrap"><canvas id="wizard-ov-canvas"></canvas></div>
        <div class="wizard-ov-thumbs" id="wizard-ov-thumbs"></div>
      </div>
      <div class="wizard-ov-panel">
        <div class="wizard-ov-tabs">
          <button type="button" class="wizard-ov-tab-btn ${tab === "results" ? "active" : ""}" data-ov-tab="results">辨識結果 <span class="wizard-ov-tab-count">${okCount}</span></button>
          <button type="button" class="wizard-ov-tab-btn ${tab === "issues" ? "active" : ""}" data-ov-tab="issues">未辨識項目 <span class="wizard-ov-tab-count warn">${issueCount}</span></button>
          <button type="button" class="wizard-ov-tab-btn ${tab === "settings" ? "active" : ""}" data-ov-tab="settings">匯入設定</button>
          <button type="button" class="btn-secondary btn-sm" id="wizard-ov-rescan-all-btn" style="margin-left:auto">↻ 重新辨識</button>
        </div>
        <div class="wizard-ov-content" id="wizard-ov-content">${wizardOverviewTabContentHtml()}</div>
      </div>
    </div>
    <div class="modal-footer">
      <button type="button" class="btn-secondary" id="wizard-ov-cancel-btn">取消</button>
      <button type="button" class="btn-primary" id="wizard-ov-confirm-all-btn">✅ 確認匯入全部(${total} 筆)</button>
    </div>`;
}

function wizardOverviewSyncTabBadges() {
  const checks = wizardComputeFieldChecks();
  const okCount = checks.filter((x) => x.ok).length;
  const issueCount = checks.length - okCount;
  const okEl = document.querySelector('.wizard-ov-tab-btn[data-ov-tab="results"] .wizard-ov-tab-count');
  const issueEl = document.querySelector('.wizard-ov-tab-btn[data-ov-tab="issues"] .wizard-ov-tab-count');
  if (okEl) okEl.textContent = okCount;
  if (issueEl) issueEl.textContent = issueCount;
}

function wireWizardOverviewCards() {
  wizardGroupList().forEach((g) => {
    const card = document.querySelector(`.wizard-ov-card[data-group-type="${g.type}"][data-group-index="${g.index}"]`);
    if (!card) return;
    card.querySelectorAll("[data-g-field]").forEach((input) => {
      input.addEventListener("input", () => {
        const field = input.dataset.gField;
        g.record[field] = input.hasAttribute("data-g-numeric") ? Number(input.value) || 0 : input.value;
        wizardOverviewSyncTabBadges();
      });
    });
    card.querySelector(".wizard-ov-delete-group")?.addEventListener("click", async () => {
      const ok = await confirmDialog(`確定要刪除這筆${g.type === "parcel" ? "地號" : "建號"}資料嗎?`, { danger: true });
      if (!ok) return;
      const list = g.type === "parcel" ? titleDeedWizard.data.parcels : titleDeedWizard.data.buildings;
      const idx = list.indexOf(g.record);
    ...
    const ownersAreaSqm =
      g.type === "parcel"
        ? Number(g.record.area_sqm) || null
        : (Number(g.record.total_area_sqm) || 0) + (g.record.accessories || []).reduce((s, a) => s + (Number(a.area_sqm) || 0), 0) ||
          Number(g.record.floor_area_sqm) ||
          null;
    renderOwnerRowsContainer(`wizard-ov-owners-${g.type}-${g.index}`, g.record.owners, g.type === "parcel" ? "lo" : "bo", ownersAreaSqm);
    if (!g.record.encumbrances) g.record.encumbrances = [];
    renderEncumbranceRows(`wizard-ov-enc-${g.type}-${g.index}`, g.record.encumbrances);
  });
}

function wireWizardOverviewTabContent() {
  if (titleDeedWizard.overviewTab === "issues") {
    document.querySelectorAll(".wizard-ov-issue-item").forEach((btn) => {
      btn.addEventListener("click", () => {
        titleDeedWizard.overviewTab = "results";
        wizardOverviewSwitchTabDom("results");
        const card = document.querySelector(
          `.wizard-ov-card[data-group-type="${btn.dataset.jumpGroupType}"][data-group-index="${btn.dataset.jumpGroupIndex}"]`
        );
        if (card) {
          card.scrollIntoView({ behavior: "smooth", block: "center" });
          card.classList.add("wizard-ov-card-flash");
          setTimeout(() => card.classList.remove("wizard-ov-card-flash"), 1500);
        }
      });
    });
    return;
  }
  if (titleDeedWizard.overviewTab === "settings") return;
  wireWizardOverviewCards();
}

function wizardOverviewSwitchTabDom(tab) {
  titleDeedWizard.overviewTab = tab;
  const content = document.getElementById("wizard-ov-content");
  if (content) content.innerHTML = wizardOverviewTabContentHtml();
  document.querySelectorAll(".wizard-ov-tab-btn").forEach((b) => b.classList.toggle("active", b.dataset.ovTab === tab));
  wireWizardOverviewTabContent();
}

async function wizardRescanAll() {
  const btn = document.getElementById("wizard-ov-rescan-all-btn");
  if (btn) {
    btn.disabled = true;
  ...
  try {
    ...
    if (!result || !result.job || result.job.status !== "completed") {
      toast((result && result.job && result.job.error_message) || "重新辨識失敗", "error");
      return;
    }
...
function renderWizardOverview() {
  titleDeedWizard.overviewTab = titleDeedWizard.overviewTab || "results";
  titleDeedWizard.viewerGlobalPage = titleDeedWizard.viewerGlobalPage || 1;
  openModal("掃描謄本匯入", wizardOverviewBodyHtml(), { width: "min(1600px, 97vw)" });
  wireWizardOverview();
  wizardRenderCurrentPage();
  wizardRenderThumbnails();
}

function wireWizardOverview() {
  const root = document.getElementById("modal-root");
  if (!root) return;

  root.querySelectorAll(".wizard-ov-tab-btn").forEach((btn) => {
    btn.addEventListener("click", () => wizardOverviewSwitchTabDom(btn.dataset.ovTab));
  });
  wireWizardOverviewTabContent();

  document.getElementById("wizard-ov-zoom-in")?.addEventListener("click", () => {
    titleDeedWizard.viewerZoom = Math.min(3, (titleDeedWizard.viewerZoom || 1) + 0.2);
    wizardRenderCurrentPage();
  });
  document.getElementById("wizard-ov-zoom-out")?.addEventListener("click", () => {
    titleDeedWizard.viewerZoom = Math.max(0.4, (titleDeedWizard.viewerZoom || 1) - 0.2);
    wizardRenderCurrentPage();
  });
  document.getElementById("wizard-ov-rotate")?.addEventListener("click", () => {
    titleDeedWizard.viewerRotation = ((titleDeedWizard.viewerRotation || 0) + 90) % 360;
    wizardRenderCurrentPage();
  });
  document.getElementById("wizard-ov-download")?.addEventListener("click", () => {
  ...
  document.getElementById("wizard-ov-rescan-all-btn")?.addEventListener("click", wizardRescanAll);
  document.getElementById("wizard-ov-cancel-btn")?.addEventListener("click", async () => {
    const ok = await confirmDialog("確定要放棄這次匯入嗎?已辨識的內容不會被儲存。", { title: "取消匯入?", confirmText: "確定取消", danger: true });
    if (!ok) return;
    closeModal();
    titleDeedWizard = null;
  });
  document.getElementById("wizard-ov-confirm-all-btn")?.addEventListener("click", confirmAndSubmitTitleDeedWizard);
}
...
function startWizardReview() {
  const d = titleDeedWizard.data;
  if (!d.parcels.length && !d.buildings.length) {
    toast("本次掃描未擷取到有效的土地地號或建物建號資料,請確認檔案類別與影像品質後重新上傳", "error");
    renderWizardStep0();
    return;
  }
  titleDeedWizard.step = 5;
  titleDeedWizard.overviewTab = "results";
  titleDeedWizard.viewerGlobalPage = 1;
  titleDeedWizard.viewerZoom = 1;
  titleDeedWizard.viewerRotation = 0;
  renderWizardStep();
}
...
async function submitTitleDeedWizardInner() {
  ...
  if (badParcelIndex !== -1) {
    toast(`第 ${badParcelIndex + 1} 筆地號缺少地號欄位,請補上`, "error");
    renderWizardOverview();
    setTimeout(() => {
      const card = document.querySelector(`.wizard-ov-card[data-group-type="parcel"][data-group-index="${badParcelIndex}"]`);
      if (card) {
        card.scrollIntoView({ behavior: "smooth", block: "center" });
        card.classList.add("wizard-ov-card-flash");
        setTimeout(() => card.classList.remove("wizard-ov-card-flash"), 1500);
