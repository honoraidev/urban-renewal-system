...
def is_shared_building_record(b) -> bool:
    """共有部分 / 公設(樓梯間、屋頂突出物等無所有權資料的建號)—— 不是任何人的
    區分所有建物,樓棟視圖不該把它當成一戶。OCR 常把每位區分所有權人的「共有
    部分:XXXX建號」誤存成掛在該人底下的一筆 building_record(同一個共有建號會
    重複幾十列),不濾掉的話那一格就會出現「×50」這種假的共有人頭數。地主清冊
    匯出也做同樣的剔除。

    注意:地下室/地下二層車位常是「由區分所有權人按持分比例個別登記」的獨立
    建號(有真實建物所有權部、各自持分 47分之1 這種),門牌只是剛好帶有「房屋
    地下N層」字樣 —— 不能只靠地址文字判斷,那樣會連同這種有真正持分人的建號
    一起濾掉。只有 main_use 明確是「共有部分」(通常伴隨「無所有權資料」)或地
    址寫明「…等共同使用」才算真正的公設。"""
