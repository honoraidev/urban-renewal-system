...
_VISION_ADDR_PROMPT = (
    ...
    "的完整地址(台灣門牌格式,如 台北市內湖區西康里6鄰內湖路一段47巷8弄34之2號)。"
    "一律使用繁體中文,不可出現簡體字;「台」不要寫成「臺」。"
...
_ADDR_AI_PROMPT = (
    ...
    "請只校正標 (OCR) 的那幾筆,輸出成台灣標準門牌格式,元件順序固定為:\n"
    "  縣市 → 區/鄉/鎮/市 → 里/村 → N鄰 → 路/街/大道 → N段 → N巷 → N弄 → N號 → N樓 → 之N\n"
    "規則:\n"
    "- 元件之間不要空格,整串連續書寫(例:台北市信義區三張里6鄰松山路391巷3弄11號2樓)。\n"
    "- 「鄰」不可省略;段用中文數字(一段/二段),樓用阿拉伯數字(2樓/11樓)。\n"
    "- 一律使用繁體中文,絕不可出現簡體字(內湖不可寫成内湖、三重區不可寫成三重区…);"
    "「台」是唯一例外,一律寫「台」不要寫成「臺」。\n"
    "- 修正字形誤認(信羲區→信義區、巿→市、叚→段…),補回明顯缺字,刪掉雜訊字。\n"
    "- 來源本來就沒有的元件(例:沒有巷弄)就不要加;遮罩符號(＊)原樣保留;"
    "無法判斷的號碼照原字輸出,絕不可臆造門牌號碼。\n"
